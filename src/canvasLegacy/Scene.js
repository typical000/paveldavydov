...
export default class DotScene {
  ...
  drawBackground() {
    ...
    gradientOverlays = new Sprite(
      DotScene.drawRadialGradient(
        x,
        y,
        x,
        y,
        this.settings.backgroundColorPrimary,
      ),
    )
    ...
    gradientOverlays = new Sprite(
      DotScene.drawRadialGradient(
        ...
        0,
        y,
        this.settings.backgroundColorSecondary,
      ),
    )
    gradientOverlays.position.x = x
    gradientOverlays.position.y = 0
  ...
  redraw() {
    ...
    for (let i = 1; i < this.settings.edges + 1; i++) {
      ...
      this.dot[prev].graphic.lineStyle(
        this.settings.dotStrokeSize,
        this.settings.dotStroke,
      )
      ...
      this.line[prev].start.x =
        this.dot[prev].position.x + this.dot[prev].graphic.x
      this.line[prev].start.y =
        this.dot[prev].position.y + this.dot[prev].graphic.y
      ...
      const controlPoint2 = this.getCoordinate(
        this.dot[prev].graphic.r - this.settings.curveDistortion,
        (i === this.settings.edges ? this.deg[0] : this.deg[i]) -
          this.curveDeformation,
      )

      // Draw lines
      this.line[prev].graphic.lineStyle(
        this.settings.strokeWidth,
        this.settings.strokeDirectLine,
      )
      this.line[prev].graphic.moveTo(
        this.line[prev].start.x,
        this.line[prev].start.y,
      )
      this.line[prev].graphic.lineTo(
      ...
      this.line[prev].graphic.lineStyle(
        this.settings.strokeWidth,
        this.settings.strokeCurvedLine,
      )
      this.line[prev].graphic.moveTo(
        this.line[prev].start.x,
        this.line[prev].start.y,
